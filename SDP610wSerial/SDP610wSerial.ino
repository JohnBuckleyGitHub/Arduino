...
void setup() {
  ...
  Serial.println("Output this line"); 
}
...
float read_SDP610(int address) {
  ...
  while(Wire.available() == 0){
    ...
    Serial.println("Wire.avialable is still:");
    Serial.println((byte) avail);
    }
  //Read the two bytes containing result:
  Serial.println("Will it print? 1.5");
  byte bHigh = Wire.read();   
  byte bLow = Wire.read(); 
  Serial.println("Will it print? 2");
